...
public class SuperEmeraldDecorator extends SuperDecorator {
                ...
                if (world.getBlock(posX, j, posY) == Block.getBlockById(2) || world.getBlock(posX, j, posY) == Block.getBlockById(3)) {
                    if (world.getBlock(posX, j + 1, posY) == Block.getBlockById(0) || world.getBlock(posX, j + 1, posY) == Block.getBlockById(9)) {
                        ...
                        for (int k = 0; k < 5; k++) {
                            ...
                            genTreeHead(world, bposX, j + startheight, bposY, cx, cy);
                        }
                        ...
                        genTreeHead(world, posX, j + height - 1, posY, cx, cy);
                        genTreeHead(world, posX + 1, j + height - 1, posY, cx, cy);
                        genTreeHead(world, posX, j + height - 1, posY + 1, cx, cy);
                        genTreeHead(world, posX + 1, j + height - 1, posY + 1, cx, cy);
                    }
                    break;
                }
            }
    ...
    private void genTreeHead(World world, int x, int y, int z, int chunkX, int chunkY){
        for (int i = -3; i < 4; i++){
            for (int j = -3; j < 4; j++){
                for (int k = 0; k < 3; k++){
