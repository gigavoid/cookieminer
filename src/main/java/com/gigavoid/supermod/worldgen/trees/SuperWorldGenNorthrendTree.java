...
public class SuperWorldGenNorthrendTree extends WorldGenAbstractTree {
    ...
    public boolean generate(World p_76484_1_, Random p_76484_2_, BlockPos pos) {
        if (p_76484_1_.getBlockState(pos.add(0, -2, 0)) == SuperBlocks.northDirt.getDefaultState()) {
    ...
    private void genTree(World world, Random random, BlockPos pos) {
        if (world.getBlockState(pos.add(0, -1, 0)) == Block.getBlockFromName("snow") && pos.getY() < 80) {
            ...
            for (int k = - 1; k < height; k++) {
                world.setBlockState(pos.add(0, k, 0), SuperBlocks.northLog.getDefaultState());
            }
            world.setBlockState(pos.add(0, height, 0), SuperBlocks.northLeaves.getDefaultState());
            ...
            for (int j = -2; j < 3; j++) {
                if ((world.getBlockState(pos.add(i, 0, j)) == Blocks.air || world.getBlockState(pos.add(i, 0, j)) == Blocks.snow_layer) && northTreeHead[(i + 2) + 5 * (j + 2)]) {
                    if (((i == -2 || i == 2) && (j == -1 || j == 1)) || ((i == -1 || i == 1) && (j == -2 || j == 2)))
                        block = .85f > r.nextFloat() ? SuperBlocks.northLeaves.getDefaultState() : Blocks.air.getDefaultState();
                    else
                        block = SuperBlocks.northLeaves.getDefaultState();
                        ...
                        switch (k) {
                            case -4:
                                if ((i == -2 || i == 2) && (j != -2 && j != 2) || (i > -2 && i < 2))
                                    if (((i == -2 || i == 2) && (j == -1 || j == 1)) || ((i == -1 || i == 1) && (j == -2 || j == 2)))
                                        block = .95f > r.nextFloat() ? SuperBlocks.northLeaves.getDefaultState() : Blocks.air.getDefaultState();
                                    else
                                        block = SuperBlocks.northLeaves.getDefaultState();
                                break;
                            case -3:
                            ...
                            case -2:
                                if (i != -2 && i != 2 && j != -2 && j != 2)
                                    if (i == -1 && j == -1 || i == -1 && j == 1 || i == 1 && j == -1 || i == 1 && j == 1)
                                        block = .9f > r.nextFloat() ? SuperBlocks.northLeaves.getDefaultState() : Blocks.air.getDefaultState();
                                    else
                                        block = SuperBlocks.northLeaves.getDefaultState();
                                break;
                            case -1:
                            ...
                            case 1:
                                if (i == 0 && j == 0)
                                    block = .75f > r.nextFloat() ? SuperBlocks.northLeaves.getDefaultState() : Blocks.air.getDefaultState();
                                break;
                        }
                    }
