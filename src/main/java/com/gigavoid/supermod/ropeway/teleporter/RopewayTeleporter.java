...
public class RopewayTeleporter extends net.minecraft.world.Teleporter
{
    ...
    public boolean func_180620_b(Entity p_180620_1_, float p_180620_2_)
    {
            ...
            for (int l = -128; l <= 128; ++l)
            {
                ...
                for (int i1 = -128; i1 <= 128; ++i1)
                {
                    for (BlockPos blockpos = blockpos4.add(l, this.worldServerInstance.getActualHeight() - 1 - blockpos4.getY(), i1); blockpos.getY() >= 0; blockpos = blockpos1)
                    {
                        ...
                        {
                            while (this.worldServerInstance.getBlockState(blockpos1 = blockpos.offsetDown()).getBlock() == RopewayBlocks.portalRopeway)
                            {
                                blockpos = blockpos1;
                            }
        ...
        if (d0 >= 0.0D)
        {
            ...
            if (this.worldServerInstance.getBlockState(((BlockPos)object).offsetWest()).getBlock() == RopewayBlocks.portalRopeway)
            {
                enumfacing = EnumFacing.NORTH;
            }

            if (this.worldServerInstance.getBlockState(((BlockPos)object).offsetEast()).getBlock() == RopewayBlocks.portalRopeway)
            {
                enumfacing = EnumFacing.SOUTH;
            }

            if (this.worldServerInstance.getBlockState(((BlockPos)object).offsetNorth()).getBlock() == RopewayBlocks.portalRopeway)
            {
                enumfacing = EnumFacing.EAST;
            }

            if (this.worldServerInstance.getBlockState(((BlockPos)object).offsetSouth()).getBlock() == RopewayBlocks.portalRopeway)
            {
